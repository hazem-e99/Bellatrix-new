...
const PagePreview = ({
  ...
  class ErrorBoundary extends React.Component {
    ...
    componentDidCatch(error, errorInfo) {
      ...
      if (this.props.componentType === "ImplementationHeroSection") {
        console.error(
          " [IMPLEMENTATION ERROR] ImplementationHeroSection render error:",
  ...
  const buildSafeProps = (props) => {
    console.log(" ========== [DEBUG START] buildSafeProps ==========");

    console.log(" [DEBUG] Input props:", JSON.parse(JSON.stringify(props)));

    console.log(" [DEBUG] Input props type:", typeof props);

    console.log(
      " [DEBUG] Input props keys:",
    ...
    commonArrayKeys.forEach((key) => {
      ...
      if (safe[key] === undefined || safe[key] === null) {
        ...
        console.log(
          ` [DEBUG] Set ${key} to empty array (was ${originalValue})`,
        );
      } else {
        console.log(` [DEBUG] Kept ${key} as:`, safe[key]);
      }
    });

    console.log(
      " [DEBUG] Final safe props:",

      JSON.parse(JSON.stringify(safe)),
    );

    console.log(" [DEBUG] Final safe props keys:", Object.keys(safe));

    console.log(" ========== [DEBUG END] buildSafeProps ==========");
  ...
  const extractComponentData = (component) => {
    ...
    // PRIORITY 2: Check if content exists (from API) - only if no form data
      ...
      console.log("ℹ [INFO] No content or contentJson found");
    }
    ...
    if (
      ...
      switch (component.componentType) {
        ...
        default:
          console.log(
            " [WARNING] No specific fallback for:",
  ...
  const renderComponent = (component, index) => {
    ...
    console.log(" [DEBUG] Calling buildSafeProps...");

    const safeProps = buildSafeProps(normalizedProps);

    console.log(
      " [DEBUG] Safe props result:",
    ...
    console.log(" [DEBUG] Adding function props...");
    ...
    if (component.componentType === "ImplementationHeroSection") {
      console.log(` [IMPLEMENTATION CHECK] ImplementationHeroSection detected`);

      console.log(
        ` [IMPLEMENTATION CHECK] Has data object:`,

        !!propsToPass.data,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] Data keys:`,

        propsToPass.data ? Object.keys(propsToPass.data) : "No data",
      );

      console.log(
        ` [IMPLEMENTATION CHECK] Has backgroundVideo:`,

        !!propsToPass.data?.backgroundVideo,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] BackgroundVideo value:`,

        propsToPass.data?.backgroundVideo,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] Has titleParts:`,

        Array.isArray(propsToPass.data?.titleParts),
      );

      console.log(
        ` [IMPLEMENTATION CHECK] TitleParts length:`,

        propsToPass.data?.titleParts?.length,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] TitleParts values:`,

        propsToPass.data?.titleParts,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] Has description:`,

        !!propsToPass.data?.description,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] Description value:`,

        propsToPass.data?.description,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] Has ctaButton:`,

        !!propsToPass.data?.ctaButton,
      );

      console.log(
        ` [IMPLEMENTATION CHECK] CTAButton structure:`,
      ...
      console.log(
        ` [IMPLEMENTATION CHECK] Full data structure:`,
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                {(() => {
                  ...
                  console.log(
                    ` [PREVIEW] Showing ${componentsToRender.length}/${pageData.components.length} visible components`,
                  );
