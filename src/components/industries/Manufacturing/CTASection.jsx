...
const CTASection = (props) => {
  // Handle both flat props and nested structures
  ...
  const features = props?.features || props?.data?.features;
  const trustedBy = props?.trustedBy || props?.data?.trustedBy;
  const openContactModal = props?.openContactModal;
  ...
  const mergedData = {
    title:
      title || finalData.title || "Transform Your Manufacturing Operations",
              ...
              {mergedData.subtitle && (
                <h3 className="text-xl md:text-2xl text-white/90 mb-4 leading-relaxed">
                  {typeof mergedData.subtitle === "string"
                    ? mergedData.subtitle
                    : mergedData.subtitle?.subtitle ||
                      mergedData.subtitle?.title ||
                      "Subtitle"}
                </h3>
              )}
