...
const DynamicPageRenderer = () => {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      try {
        ...
        // Load components for each section (prefer new components format, fall back to legacy sections)
        ...
        if (
          ...
          sectionsToRender = pageData.components;
          console.log(
            " [COMPONENT LOADING] Using modern components array (legacy sections ignored)",
          ...
          sectionsToRender = pageData.sections;
          console.log(
            " [COMPONENT LOADING] Falling back to legacy sections array",
            { count: sectionsToRender.length },
          );
        } else {
          console.warn(
            " [COMPONENT LOADING] No components or sections found in page data",
            { components: pageData.components, sections: pageData.sections },
          );
        }

        console.log(
          " [COMPONENT LOADING] Original components:",
        ...
        if (Array.isArray(sectionsToRender) && sectionsToRender.length > 0) {
          ...
          sectionsToRender = sectionsToRender.filter((section) => {
            const isVisible =
              section.isVisible === true || section.isVisible === 1;

            console.log(
              ` [LOADING FILTER] ${section.componentName || section.componentType}: isVisible=${section.isVisible} (${typeof section.isVisible}) -> ${isVisible ? "LOADING" : "SKIPPING"}`,
            );
          ...
          console.log(
            ` [LOADING RESULT] ${beforeFilter} -> ${sectionsToRender.length} components will be loaded`,
          );
          ...
          if (sectionsToRender.length === 0) {
            console.warn(
              " [WARNING] No visible components to load! All components may be hidden.",
            );
          }
        } else {
          console.warn(" [WARNING] No components found for loading");
        }
  ...
  const getFilteredComponents = () => {
    ...
    console.log(
      " [DYNAMIC RENDERER] Original components:",
      sectionsToRender.map((c) => ({
        name: c.componentName || c.componentType,
    ...
    if (Array.isArray(sectionsToRender) && sectionsToRender.length > 0) {
      ...
      sectionsToRender = sectionsToRender.filter((section) => {
        ...
        console.log(
          ` [FILTER] ${section.componentName || section.componentType}: isVisible=${section.isVisible} (${typeof section.isVisible}) -> ${isVisible ? "INCLUDED" : "EXCLUDED"}`,
        );
      ...
      console.log(
        ` [FILTER RESULT] ${beforeFilter} -> ${sectionsToRender.length} components`,
      );
      ...
      if (sectionsToRender.length === 0) {
        console.warn(
          " [WARNING] No visible components found! All components may be hidden.",
        );
      }
    } else {
      console.warn(" [WARNING] No components found in pageData");
    }
    ...
    <div>
      ...
      {filteredComponents.length > 0 ? (
        filteredComponents.map((section, index) => {
          console.log(
            ` [RENDERING] Component ${index + 1}: ${section.componentName || section.componentType}`,
          );
